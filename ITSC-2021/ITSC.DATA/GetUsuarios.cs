        ...
        public static IEnumerable<SedeCarrera> GetCarreras(int dni)
        {
            using(var db = new ITSCContext())
            {
                ...
                IEnumerable<SedeCarrera> carreras_al = from c in db.sedes_carreras
                                             join m in db.materias_carreras on c.id equals m.sedecarrera_id
                                             join cr in db.cursantes on m.id equals cr.materiacarrera_id
                                             join al in db.alumnos on cr.alumno_id equals al.id
                                             where al.dni == dni
                                             select c;
